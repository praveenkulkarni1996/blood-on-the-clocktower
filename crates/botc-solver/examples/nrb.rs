...
use botc_core::{
    Character, Claim, Demon, Evil, Good, Minion, Outsider, Player, ReportLog, Townsfolk,
};

#[allow(unused_variables)]
fn setup_game() -> Vec<ReportLog> {
    use Claim::{
        Am, ChefGets, FortuneTellerNo, FortuneTellerYes, LibrarianSees, PoisonerPoisons,
        UndertakerSees,
    };
    use Outsider::Recluse;
    use ReportLog::{DayExecutes, DocumentOnly, NightKilled, OnTime};
    ...
    vec![
        ...
        DocumentOnly(Night(1), sullivan, PoisonerPoisons(isaac)),
        //
        ...
        OnTime(Night(1), carly, FortuneTellerYes(adam, laurie)),
        ...
        DayExecutes(Day(1), adam),
        // NIGHT-2
        // 1. Sullivan (Poisoner) poisons Brooke (the Undertaker).
        DocumentOnly(Night(2), sullivan, PoisonerPoisons(brooke)),
        ...
        OnTime(Night(2), carly, FortuneTellerYes(carly, sullivan)),
        // 5. Brooke (Undertaker - Drunk) sees Adam as the chef.
        OnTime(
            Night(2),
            brooke,
            UndertakerSees(adam, Character::Good(Good::Townsfolk(Townsfolk::Chef))),
        ),
        // DAY-2
        ...
        DocumentOnly(Night(4), sullivan, PoisonerPoisons(carly)),
        ...
        DocumentOnly(Night(5), sullivan, PoisonerPoisons(dom)),
        NightKilled(Night(5), dom),
        // Claims
        OnTime(
            Night(1),
            ollie,
            Am(Character::Good(Good::Townsfolk(Townsfolk::Soldier))), /* Lying. Ollie is
                                                                       * a Baron. */
        ),
        OnTime(
            Night(1),
            carly,
            Am(Character::Good(Good::Townsfolk(Townsfolk::FortuneTeller))),
        ),
        OnTime(
            Night(1),
            john,
            Am(Character::Good(Good::Townsfolk(Townsfolk::Librarian))),
        ),
        OnTime(
            Night(1),
            brooke,
            Am(Character::Good(Good::Townsfolk(Townsfolk::Undertaker))),
        ),
        OnTime(
            Night(1),
            adam,
            Am(Character::Good(Good::Townsfolk(Townsfolk::Chef))),
        ),
        OnTime(
            Night(1),
            isaac,
            Am(Character::Good(Good::Townsfolk(Townsfolk::Monk))),
        ),
        OnTime(
            Night(1),
            sullivan,
            Am(Character::Good(Good::Townsfolk(Townsfolk::Empath))), /* Lying: Actually
                                                                      * poisoner */
        ),
        OnTime(
            Night(1),
            laurie,
            Am(Character::Good(Good::Outsider(Outsider::Recluse))),
        ),
        OnTime(
            Night(1),
            dom,
            Am(Character::Good(Good::Townsfolk(Townsfolk::Mayor))),
        ),
        OnTime(
            Night(1),
            blair,
            Am(Character::Good(Good::Outsider(Outsider::Saint))), // Lying
        ),
    ]
}
...
fn main() {
    ...
    for seat in 0..10 {
        ...
        solver.assert(imp_var);

        match solver.check() {
            z3::SatResult::Sat => {
                println!("=== SAT WORLD: Seat({seat}) is the Imp ===");
            ...
            z3::SatResult::Unsat => {
                println!("=== UNSAT: Seat({seat}) cannot be the Imp ===");
            }
            other @ z3::SatResult::Unknown => {
