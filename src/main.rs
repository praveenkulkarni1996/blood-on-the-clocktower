...
fn setup_game() {
    ...
    let logs = vec![
        ...
        ReportLog::OnTime(isaac, Log::MonkProtects(adam)),
        // 3. Carly (Fortune Teller) selects herself and Dom. (No - Poisoned)
        ReportLog::OnTime(carly, Log::FortuneTellerLearns(carly, dom, false)),
        // 4. Blair (Imp) kills Brooke (the Drunk).
        ReportLog::OnTime(blair, Log::ImpKills(brooke)),
        // DAY-3
        // Isaac (the Monk) is nominated and executed.
        ReportLog::OnTime(Player::Unresolved, Log::Executes(isaac)),
        // NIGHT-4
